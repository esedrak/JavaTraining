 ...
 *
 * <p>Demonstrates: - Classic instanceof + explicit cast - Pattern matching instanceof (Java 16+):
 * if (obj instanceof String s) - ClassCastException when casting incompatible types - Describing
...
public class TypeCheckingAndCasting {
  ...
  /**
   * Modern approach (Java 16+): pattern matching for instanceof. The variable 's' is in scope only
   * within the if-block.
   */
  ...
  /**
   * Demonstrates a ClassCastException when an illegal cast is attempted. In practice you'd always
   * check with instanceof first — this shows what happens if you don't.
   */
